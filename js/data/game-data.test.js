...
describe(`gameData`, () => {
  describe(`getUserAnswers`, () => {
    ...
    const normalAnswers = Array(Config.NUMBER_OF_QUESTIONS).fill({correct: true, speed: `normal`, type: AnswerType.CORRECT});

    const badAnswers = Array(Config.NUMBER_OF_QUESTIONS).fill({correct: true, speed: `slow`, type: AnswerType.SLOW});
    badAnswers.fill({correct: false, speed: `slow`, type: AnswerType.SLOW}, 0, 3);

    const bestAnswers = Array(Config.NUMBER_OF_QUESTIONS).fill({correct: true, speed: `fast`, type: AnswerType.FAST});
    ...
    it(`should return -1, if user didn't answer all 10 questions`, () => {
      ...
      assert.strictEqual(countStats(normalAnswers.slice(0, Config.NUMBER_OF_QUESTIONS - 1), Config.LIVES), -1);
    });
    ...
    it(`should return 350, if user gave all answers with slow speed and lost all lives`, () => {
      assert.strictEqual(countStats(badAnswers, Config.MIN_LIVES).total, 350);
    });

    it(`should return 1650, if user gave all answers with fast speed and saved all lives`, () => {
      assert.strictEqual(countStats(bestAnswers, Config.LIVES).total, 1650);
    });
